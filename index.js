...
const { getApiStatus } = require('./apiFootball');
...
// ── 查 API-Football 狀態與額度（admin）
if (!isGroup && user.is_admin && (text === 'API狀態' || text === 'API額度')) {
  if (!API_FOOTBALL_KEY) {
    return client.replyMessage(event.replyToken, {
      type: 'text',
  ...
  const st = await getApiStatus({ apiKey: API_FOOTBALL_KEY });
  const { rate } = st;
  const fmt = (v) => (v == null ? '—' : v);
  const dayUsed = (rate.dayLimit != null && rate.dayRemaining != null)
    ? rate.dayLimit - rate.dayRemaining : null;

  const lines = ['📊 API-Football 狀態', ''];

  if (st.ok) {
    const sub = st.subscription || {};
    lines.push(
      `帳號：${[st.account?.firstname, st.account?.lastname].filter(Boolean).join(' ') || '—'}`,
      `Email：${st.account?.email || '—'}`,
      `方案：${sub.plan || '—'}（${sub.active ? '啟用中' : '未啟用'}）`,
      `到期：${sub.end || '—'}`,
    );
  } else {
    lines.push(`❌ 帳號異常：${st.error || '無法取得狀態'}`);
  }

  lines.push(
    '',
    `今日額度：${fmt(dayUsed)}/${fmt(rate.dayLimit)} 次（剩 ${fmt(rate.dayRemaining)}）`,
    `每分鐘：剩 ${fmt(rate.minRemaining)}/${fmt(rate.minLimit)} 次`,
  );
